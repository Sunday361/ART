 ...
 */
    ...
    public:
        ...
        static VarlenEntry Create(const byte *content, uint32_t size, bool reclaim) {
            ...
            ASSERT(size > InlineThreshold(), "small varlen values should be inlined");
        ...
        static VarlenEntry CreateInline(const byte *content, uint32_t size) {
            ASSERT(size <= InlineThreshold(), "varlen value must be small enough for inlining to happen");
            VarlenEntry result;
            result.size_ = size;
        ...
        std::vector<T> DeserializeArray() const {
            ...
            ASSERT(sizeof(T) == (Size() - sizeof(size_t)) / num_elements,
